  ...
  const handleAI = async (code) => {
    if (code) {
        try {
            ...
            setIsLoading(false);
            console.log('received data', test_cases);
            
            // Push inputs and expected outputs
            for (let testcase of test_cases) {
                inputCollection.push(testcase.input);
                outputCollection.push(testcase.expected_output);
            }
  ...
  return (
    <div className="my-4 flex flex-col">
      <div className="w-full flex gap-4">
        <textarea
          rows="2"
          className="bg-surface-a90 text-gray-300 w-full h-fit my-2 p-2 font-code rounded focus:border-opacity-0 focus:outline-none focus:ring-1 focus:ring-blue-500"
      ...
      <div className="w-full px-4 pr-0 py-2 mt-4  xs:flex-row xs:items-center  md:flex md:gap-4 md:justify-end ">
        <button
          ...
          className="w-fit h-8 px-1 pr-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md xs:mb-4 md:mb-0 md:text-base"
        >
        ...
        <button
          className="px-4 py-2 border border-gold"
